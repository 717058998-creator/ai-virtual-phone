...
export function editAndResendQaMessage(
    ...
    const ghConfig = loadQaGithubConfig();
    for (const msg of discardedMessages) {
        if (ghConfig && msg.pendingCommit?.status === "applied" && msg.pendingCommit.result) {
            try {
                void revertQaCommit(ghConfig, msg.pendingCommit.result);
            } catch {
                // ignore
            }
        }
